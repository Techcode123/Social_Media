...
export const UserCard = (prop) => {
    ...
    return (
        <>
            <div className="flex w-full  border border-gray-400 p-2 rounded-md justify-between mt-2 " style={prop?.id === userId ? { display: "none" } : { display: "flex" }}>
                <div className="flex">
                    <div className=" w-[50px] lg:h-[50px] rounded-full border border-gray-400 mr-2 cursor-pointer " onClick={handleUser}>
                ...
                {loading ?
                    ...
                    prop?.friends?.includes(userId) ? "" : prop?.requestarrive?.includes(userId) ? <div >
                        <i class="fa-solid fa-check" style={{ color: "#14d299" }}></i>
                    </div> : <div className="cursor-pointer" title="Add Friend" onClick={handleRequest}>
